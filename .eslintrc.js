module.exports = {
  ...
  rules: {
    ...
    'vue/max-attributes-per-line': [2, {
      'singleline': 10, // 单行时可接受的最大数量
      'multiline': {
        'max': 1, // 多行时可接受的最大数量
        // 'allowFirstLine': false, // 允许属性和标记名在同一行（此属性8.0被移除）
      },
    }],
    // 禁用单行标签内容需换行的校验
    ...
    // 组件 name 属性值必须使用帕斯卡命名法（大驼峰）
    // 'vue/name-property-casing': ['error', 'PascalCase'],
    // 禁用 禁止使用v-html
    'vue/no-v-html': 'off',
    // 对象必须使用 getter/setter , 每个已定义setter的属性都必须有一个getter
    'accessor-pairs': 2,
    // 箭头函数 => 前后必须有空格
    'arrow-spacing': [2, {
      'before': true,
    ...
    // 强制执行标准大括号风格样式：关键字与花括号保持在同一行
    'brace-style': [
      2,
      '1tbs',
      {
        'allowSingleLine': true, // 允许单行括号
      }
    ],
    ...
    'camelcase': [
      0,
      {
        'properties': 'always', // 属性名称强制执行驼峰命名
      }
    ],
    // 在对象或数组中不允许尾随逗号
